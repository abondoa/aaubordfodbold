...
if(!isset($_GET['id'])){
    ...
    $teamsResult = $DB->query(" SELECT sum(wins), sum(losses) 
                                FROM teams JOIN memberof 
                                  ON teams.teamID = memberof.teamID 
                                WHERE memberof.playerID = " . $id . " GROUP BY memberof.playerID");
